...
//* RCLAB modified
void vfillzero(float *a, int n) {
    for (int i = 1; i <= n; ++i) {
        a[i] = 0.0;
//        printf("%f \n", a[i]);
    }
}
void mfillzero(float **a, int a_row, int a_col) {
...
void mcopy( float **a, int a_rows, int a_cols, float **b) /* copy a matrix */
{
	int i, j;

#ifdef V_CHECK
	if ( !valid_matrix_b( a ) )
		nrerror("Invalid matrix: mcopy\n");
	if ( !valid_matrix_b( b ) )
      nrerror("Invalid destination matrix: mcopy\n");    
#endif
...
void mmult( float **a, int a_rows, int a_cols, 
            float **b, int b_rows, int b_cols, float **y)
/* multiply two matrices a, b, result in y. y must not be same as a or b */
{
   int i, j, k;
   float sum;
   ...
   if ( a_cols != b_rows ) {
      fprintf(stderr,"a_cols <> b_rows (%d,%d): mmult\n", a_cols, b_rows);
      exit(1);
   }

#ifdef V_CHECK
   if ( !valid_matrix_b( a ) ) 
      nrerror("Invalid 1st matrix: mmult\n");    
   if ( !valid_matrix_b( b ) ) 
      nrerror("Invalid 2nd matrix: mmult\n");    
   if ( !valid_matrix_b( y ) ) 
      nrerror("Invalid result matrix: mmult\n"); 
#endif
...
void mvmult( float **a, int a_rows, int a_cols, float *b, int b_els, float *y)
/* multiply a matrix a by vector b, result in y. y can be same as b */
{
   int i, k;
   float sum;
...
#ifdef V_CHECK
   if ( !valid_matrix_b( a ) ) 
      nrerror("Invalid matrix: mvmult\n");   
   if ( !valid_vector_b( b ) ) 
      nrerror("Null pointer to vector: mvmult");
#endif
...
void madd( float **a, int a_rows, int a_cols, float **b, float **y)
...
#ifdef V_CHECK
   if ( !valid_matrix_b( a ) ) 
      nrerror("Invalid 1st matrix: madd\n"); 
   if ( !valid_matrix_b( b ) ) 
      nrerror("Invalid 2nd matrix: madd\n"); 
   if ( !valid_matrix_b( y ) ) 
      nrerror("Invalid result matrix: madd\n");  
#endif
...
void msub( float **a, int a_rows, int a_cols, float **b, float **y)
...
#ifdef V_CHECK
   if ( !valid_matrix_b( a ) ) 
      nrerror("Invalid 1st matrix: msub\n"); 
   if ( !valid_matrix_b( b ) ) 
      nrerror("Invalid 2nd matrix: msub\n"); 
   if ( !valid_matrix_b( y ) ) 
      nrerror("Invalid result matrix: msub\n");  
#endif
...
void msmy( float **a, int a_rows, int a_cols, float r, float **y)
...
#ifdef V_CHECK
   if ( !valid_matrix_b( a ) ) 
      nrerror("Invalid 1st matrix: msmy\n"); 
   if ( !valid_matrix_b( y ) ) 
      nrerror("Invalid result matrix: msmy\n");  
#endif
...
void mtranspose( float **a, int a_rows, int a_cols, float **y)
...
#ifdef V_CHECK
   if ( !valid_matrix_b( a ) ) 
      nrerror("Invalid 1st matrix: mtranspose\n");   
   if ( !valid_matrix_b( y ) ) 
      nrerror("Invalid result matrix: mtranspose\n");    
#endif
...
void vcopy( float *a, int a_els, float *y) /* copy a vector */
{
   int i;

#ifdef V_CHECK
   if ( !valid_vector_b( a ) ) 
      nrerror("Null pointer to 1st vector: vcopy");
   if ( !valid_vector_b( y ) ) 
      nrerror("Null pointer to 2nd vector: vcopy");
#endif  

   for ( i=1; i<=a_els; i++ ) y[i] = a[i];
}

void vadd( float *a, int a_els, float *b, float *y)
{
   int j;

#ifdef V_CHECK
   if ( !valid_vector_b( a ) ) 
      nrerror("Null pointer to 1st vector: vadd");
   if ( !valid_vector_b( b ) ) 
      nrerror("Null pointer to 2nd vector: vadd");
   if ( !valid_vector_b( y ) ) 
      nrerror("Null pointer to result vector: vadd");
#endif
   for ( j=1; j<=a_els; j++ ) {
      y[j] = a[j] + b[j];
   }
...
void vsub( float *a, int a_els, float *b, float *y)
{
   int j;

#ifdef V_CHECK
   if ( !valid_vector_b( a ) ) 
      nrerror("Null pointer to 1st vector: vsub");
   if ( !valid_vector_b( b ) ) 
      nrerror("Null pointer to 2nd vector: vsub");
   if ( !valid_vector_b( y ) ) 
      nrerror("Null pointer to result vector: vsub");
#endif
...
float vdot( float *a, int a_els, float *b)
{
   int j;
   double y;
...
#ifdef V_CHECK
   if ( !valid_vector_b( a ) ) 
      nrerror("Null pointer to 1st vector: vdot");
   if ( !valid_vector_b( b ) ) 
      nrerror("Null pointer to 2nd vector: vdot");
#endif
...
#ifdef V_CHECK
   if ( !valid_vector_b( a ) ) 
      nrerror("Null pointer to vector: vmag");
#endif
...
void vsmy( float *a, int a_els, float r, float *y)
{
   int j;
