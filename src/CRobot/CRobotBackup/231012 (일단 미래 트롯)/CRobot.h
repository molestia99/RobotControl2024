...
typedef struct EndPoint {
    ...
    float* G_Base2Foot_RefPos                = fvector(1, 3);   // 내 생각엔, 이 어법으로 메모리를 동적으로 만든 뒤 삭제를 해주지 않아서
    float* G_Base2Foot_RefVel                = fvector(1, 3);   // 리눅스 상 시뮬레이션을 여러번 반복하다보면 컴퓨터가 맛이 가는 것 같다.
...
typedef struct Walk_Parameter_new{
    unsigned int gait_type = MOVE_WALK; // 0: WALK, 1: TROT
    unsigned int gait_type_target = MOVE_WALK;
    ...
    // future traj
    Vector3f p_CoM_future1_traj = VectorXf::Zero(3);
    Vector3f p_FL_future1_traj = VectorXf::Zero(3);
    Vector3f p_FR_future1_traj = VectorXf::Zero(3);
    Vector3f p_RL_future1_traj = VectorXf::Zero(3);
    Vector3f p_RR_future1_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future1 = VectorXf::Zero(4);

    Vector3f p_CoM_future2_traj = VectorXf::Zero(3);
    Vector3f p_FL_future2_traj = VectorXf::Zero(3);
    Vector3f p_FR_future2_traj = VectorXf::Zero(3);
    Vector3f p_RL_future2_traj = VectorXf::Zero(3);
    Vector3f p_RR_future2_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future2 = VectorXf::Zero(4);
    

    Vector3f p_CoM_future3_traj = VectorXf::Zero(3);
    Vector3f p_FL_future3_traj = VectorXf::Zero(3);
    Vector3f p_FR_future3_traj = VectorXf::Zero(3);
    Vector3f p_RL_future3_traj = VectorXf::Zero(3);
    Vector3f p_RR_future3_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future3 = VectorXf::Zero(4);

    Vector3f p_CoM_future4_traj = VectorXf::Zero(3);
    Vector3f p_FL_future4_traj = VectorXf::Zero(3);
    Vector3f p_FR_future4_traj = VectorXf::Zero(3);
    Vector3f p_RL_future4_traj = VectorXf::Zero(3);
    Vector3f p_RR_future4_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future4 = VectorXf::Zero(4);

    Vector3f p_CoM_future5_traj = VectorXf::Zero(3);
    Vector3f p_FL_future5_traj = VectorXf::Zero(3);
    Vector3f p_FR_future5_traj = VectorXf::Zero(3);
    Vector3f p_RL_future5_traj = VectorXf::Zero(3);
    Vector3f p_RR_future5_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future5 = VectorXf::Zero(4);

    Vector3f p_CoM_future6_traj = VectorXf::Zero(3);
    Vector3f p_FL_future6_traj = VectorXf::Zero(3);
    Vector3f p_FR_future6_traj = VectorXf::Zero(3);
    Vector3f p_RL_future6_traj = VectorXf::Zero(3);
    Vector3f p_RR_future6_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future6 = VectorXf::Zero(4);

    Vector3f p_CoM_future7_traj = VectorXf::Zero(3);
    Vector3f p_FL_future7_traj = VectorXf::Zero(3);
    Vector3f p_FR_future7_traj = VectorXf::Zero(3);
    Vector3f p_RL_future7_traj = VectorXf::Zero(3);
    Vector3f p_RR_future7_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future7 = VectorXf::Zero(4);

    Vector3f p_CoM_future8_traj = VectorXf::Zero(3);
    Vector3f p_FL_future8_traj = VectorXf::Zero(3);
    Vector3f p_FR_future8_traj = VectorXf::Zero(3);
    Vector3f p_RL_future8_traj = VectorXf::Zero(3);
    Vector3f p_RR_future8_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future8 = VectorXf::Zero(4);

    Vector3f p_CoM_future9_traj = VectorXf::Zero(3);
    Vector3f p_FL_future9_traj = VectorXf::Zero(3);
    Vector3f p_FR_future9_traj = VectorXf::Zero(3);
    Vector3f p_RL_future9_traj = VectorXf::Zero(3);
    Vector3f p_RR_future9_traj = VectorXf::Zero(3);
    Vector4f contact_foots_future9 = VectorXf::Zero(4);

    MatrixXf contact_foots_total = MatrixXf::Zero(4, horizontal_length);

    VectorXf p_CoM2FL_traj_future = VectorXf::Zero(horizontal_length * 3);
    VectorXf p_CoM2FR_traj_future = VectorXf::Zero(horizontal_length * 3);
    VectorXf p_CoM2RL_traj_future = VectorXf::Zero(horizontal_length * 3);
    VectorXf p_CoM2RR_traj_future = VectorXf::Zero(horizontal_length * 3);
...
public:
    ...
    void Optimization_Process(const unsigned int gait_type, const VectorXf vel_com_target, float& T_period, float& T_stance, float& T_swing, Vector3f* vel_com_desired);
    ...
    VectorXf temp_Foot_to_CoM_ref = VectorXf::Zero(3*200);  // global 좌표계에서 com의 미래 레퍼런스 모음인 temp_com_ref 를 "다리 좌표계" 에서 관측하기 위한 값
    VectorXf temp_Foot_to_CoM_vel_ref = VectorXf::Zero(3*200);
