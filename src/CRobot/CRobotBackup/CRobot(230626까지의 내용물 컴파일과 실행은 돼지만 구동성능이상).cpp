...
void CRobot::initializeOSQP(void) {
    ...
    */
    ...
    for(size_t i = 0; i < horizontal_length; ++i){
        ...
        osqp.B_c.block<3, 3>(13*i + 6, 0) = osqp.w_I_g_transpose * skew_matrix(osqp.r_fl_vector.block<3, 1>(3*i, 0)) * osqp.w_I_g;
        osqp.B_c.block<3, 3>(13*i + 6, 3) = osqp.w_I_g_transpose * skew_matrix(osqp.r_fr_vector.block<3, 1>(3*i, 0)) * osqp.w_I_g;
        osqp.B_c.block<3, 3>(13*i + 6, 6) = osqp.w_I_g_transpose * skew_matrix(osqp.r_rl_vector.block<3, 1>(3*i, 0)) * osqp.w_I_g;
        osqp.B_c.block<3, 3>(13*i + 6, 9) = osqp.w_I_g_transpose * skew_matrix(osqp.r_rr_vector.block<3, 1>(3*i, 0)) * osqp.w_I_g;
...
void CRobot::ProcessOSQP(void) {   
    ...
    */
    ...
    for(size_t i = 0; i < horizontal_length; ++i){
        // B_c 는 13*(3*4) 의 행렬이 l 개의 행으로 구성된 (13*l) * (3*4) 행렬.
        osqp.B_c.block<3, 3>(13*i + 6, 0) = osqp.w_I_g_transpose * skew_matrix(osqp.r_fl_vector.block<3, 1>(3*i, 0));
        osqp.B_c.block<3, 3>(13*i + 6, 3) = osqp.w_I_g_transpose * skew_matrix(osqp.r_fr_vector.block<3, 1>(3*i, 0));
        osqp.B_c.block<3, 3>(13*i + 6, 6) = osqp.w_I_g_transpose * skew_matrix(osqp.r_rl_vector.block<3, 1>(3*i, 0));
        osqp.B_c.block<3, 3>(13*i + 6, 9) = osqp.w_I_g_transpose * skew_matrix(osqp.r_rr_vector.block<3, 1>(3*i, 0));
...
//                    walking.step.count                  = walking.step.RLFR_SWING_START-onesecScale*tasktime;
    ...
    for(size_t i = 1; i < 100; ++i){ // 100 은 미래를 바라보려는 갯수. 추후 Trot 함수의 디폴트 매개변수를 따로 만들어주어야 할 것이다.
        ...
        temp_com_ref.block<3, 1>(i*3, 0) = generateCoMTraj_modified(com_motion_time, i);
...
void CRobot::SaveData() {
    ...
    print.Data(342)     = temp_Foot_to_CoM_ref(3);
    print.Data(343)     = temp_Foot_to_CoM_ref(30); //temp_Foot_to_CoM_ref
