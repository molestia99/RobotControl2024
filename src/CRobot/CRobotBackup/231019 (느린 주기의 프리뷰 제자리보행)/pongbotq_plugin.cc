...
double time_5s_to_8s[500];
double time_8s_to_15s[1167];
double time_15s_to_22s[1167];

double CoM_x_5s_to_8s[500];
double CoM_x_8s_to_15s[1167];
double CoM_x_15s_to_22s[1167];

double CoM_y_5s_to_8s[500];
double CoM_y_8s_to_15s[1167];
double CoM_y_15s_to_22s[1167];
...
void gazebo::PongBot_plugin::Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/) {    
    ...
    ifstream input_5s_to_8s;
    ifstream input_8s_to_15s;
    ifstream input_15s_to_22s;
    input_5s_to_8s.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/data_until3s.txt");
    input_8s_to_15s.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/data_3s_to_10s.txt");
    input_15s_to_22s.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/data_10s_to_17s.txt");

    for(size_t i = 0; i < 500; ++i){
        input_5s_to_8s >> time_5s_to_8s[i];
        input_5s_to_8s >> CoM_x_5s_to_8s[i];
        input_5s_to_8s >> CoM_y_5s_to_8s[i];
    }

    for(size_t i = 0; i < 1167; ++i){
        input_8s_to_15s >> time_8s_to_15s[i];
        input_8s_to_15s >> CoM_x_8s_to_15s[i];
        input_8s_to_15s >> CoM_y_8s_to_15s[i];

        input_15s_to_22s >> time_15s_to_22s[i];
        input_15s_to_22s >> CoM_x_15s_to_22s[i];
        input_15s_to_22s >> CoM_y_15s_to_22s[i];
    }
        ...
        if(PongBot.osqp.Flag == true){
            ...
            if(time > 5.){
                ...
                if(time > 8. && time <= 15.){
                    ...
                    PongBot.walk_para_new.p_CoM_traj(AXIS_X) = CoM_x_8s_to_15s[index] * 1.;
                    PongBot.walk_para_new.p_CoM_traj(AXIS_Y) = CoM_y_8s_to_15s[index] * 1.5;
                    ++i;
                    if(i > 3500) i = 3500;                   // 500 * 3
                } 
                    ...
                    PongBot.walk_para_new.p_CoM_traj << 0., 0., 0.4;
                    size_t index = static_cast<size_t>(i /3);
                    PongBot.walk_para_new.p_CoM_traj(AXIS_X) = CoM_x_15s_to_22s[index] * 1.;
                    PongBot.walk_para_new.p_CoM_traj(AXIS_Y) = CoM_y_15s_to_22s[index] * 1.5;
                    ++i;
                    if(i > 3500) i = 0;                   // 500 * 3
                }               
                ...
                if(time <= 8.){
                    ...
                    PongBot.walk_para_new.p_CoM_traj << 0., 0., 0.4;
                    size_t index = static_cast<size_t>(i /3);
                    PongBot.walk_para_new.p_CoM_traj(AXIS_X) = CoM_x_5s_to_8s[index] * 1.;
                    PongBot.walk_para_new.p_CoM_traj(AXIS_Y) = CoM_y_5s_to_8s[index] * 1.5;
                    ++i;
