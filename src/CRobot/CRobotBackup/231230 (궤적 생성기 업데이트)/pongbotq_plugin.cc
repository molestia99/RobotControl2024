...
// double time_begin[500];
// double time_middle[500];
// double time_after[500];

// double CoM_x_begin[500];
// double CoM_x_middle[500];
// double CoM_x_after[500];

// double CoM_y_begin[500];
// double CoM_y_middle[500];
// double CoM_y_after[500];

unsigned int data_size_dataset = 58429;

float time_dataset[58429];
float CoM_x_dataset[58429];
float CoM_y_dataset[58429];
float CoM_z_dataset[58429];
float FL_x_dataset[58429];
float FL_y_dataset[58429];
float FL_z_dataset[58429];
float FR_x_dataset[58429];
float FR_y_dataset[58429];
float FR_z_dataset[58429];
float RL_x_dataset[58429];
float RL_y_dataset[58429];
float RL_z_dataset[58429];
float RR_x_dataset[58429];
float RR_y_dataset[58429];
float RR_z_dataset[58429];

bool contact_FL_dataset[58429];
bool contact_FR_dataset[58429];
bool contact_RL_dataset[58429];
bool contact_RR_dataset[58429];

bool phase_dataset[58429];

float yaw_dataset[58429];
float yaw_traj;
...
void gazebo::PongBot_plugin::Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/) {    
    ...
    // test arrayinput
    ifstream inputfile;
    inputfile.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/inputTest.txt");
    ...
    for(size_t i = 0; i < 5; ++i){
        ...
        inputfile >> testArray2[i];
    }

    for(size_t i = 0; i < 5; ++i){
        // std::cout << "test: testArray[" << i << "]: " << testArray[i] << std::endl;
        // std::cout << "test: testArray2[" << i << "]: " << testArray2[i] << std::endl;
    }



    // double time_begin[500];
    // double time_middle[1167];
    // double time_after[1167];

    // double CoM_x_begin[500];
    // double CoM_x_middle[1167];
    // double CoM_x_after[1167];

    // double CoM_y_begin[500];
    // double CoM_y_middle[1167];
    // double CoM_y_after[1167];
    
    // ifstream input_begin;
    // ifstream input_middle;
    // ifstream input_after;

    // input_begin.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/data_input.txt");
    // input_middle.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/data_middle.txt");
    // input_after.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/data_after.txt");

    

    // for(size_t i = 0; i < 500; ++i){
    //     input_begin >> time_begin[i];
    //     input_begin >> CoM_x_begin[i];
    //     input_begin >> CoM_y_begin[i];
    // }

    // for(size_t i = 0; i < 500; ++i){
    //     input_middle >> time_middle[i];
    //     input_middle >> CoM_x_middle[i];
    //     input_middle >> CoM_y_middle[i];

    //     input_after >> time_after[i];
    //     input_after >> CoM_x_after[i];
    //     input_after >> CoM_y_after[i];
    // }

    ifstream whole_dataset;
    whole_dataset.open("/home/wonsukji/catkin_ws/src/RcLab-PongBotQ-develop_WsJi/src/dataset_231026_pongbot_sim02.txt");
    if(!whole_dataset){
        std::cout << "open failed" << std::endl;
    }

    // float CoM_x_dataset[29504];
    // float CoM_y_dataset[29504];
    // float CoM_z_dataset[29504];
    // float FL_x_dataset[29504];
    // float FL_y_dataset[29504];
    // float FL_z_dataset[29504];
    // float FR_x_dataset[29504];
    // float FR_y_dataset[29504];
    // float FR_z_dataset[29504];
    // float RL_x_dataset[29504];
    // float RL_y_dataset[29504];
    // float RL_z_dataset[29504];
    // float RR_x_dataset[29504];
    // float RR_y_dataset[29504];
    // float RR_z_dataset[29504];

    // bool contact_FL_dataset[29504];
    // bool contact_FR_dataset[29504];
    // bool contact_RL_dataset[29504];
    // bool contact_RR_dataset[29504];

    // bool phase_dataset[29504];

    for(size_t i = 0; i < 58429; ++i){
        // whole_dataset >> time_dataset[i];
        whole_dataset >> CoM_x_dataset[i];
        whole_dataset >> CoM_y_dataset[i];
        CoM_z_dataset[i] = 0.4;
        
        whole_dataset >> FL_x_dataset[i];
        whole_dataset >> FL_y_dataset[i];
        whole_dataset >> FL_z_dataset[i];

        whole_dataset >> FR_x_dataset[i];
        whole_dataset >> FR_y_dataset[i];
        whole_dataset >> FR_z_dataset[i];

        whole_dataset >> RL_x_dataset[i];
        whole_dataset >> RL_y_dataset[i];
        whole_dataset >> RL_z_dataset[i];

        whole_dataset >> RR_x_dataset[i];
        whole_dataset >> RR_y_dataset[i];
        whole_dataset >> RR_z_dataset[i];

        whole_dataset >> yaw_dataset[i];

        whole_dataset >> contact_FL_dataset[i];
        whole_dataset >> contact_FR_dataset[i];
        whole_dataset >> contact_RL_dataset[i];
        whole_dataset >> contact_RR_dataset[i];

        // whole_dataset >> phase_dataset[i];
    }

    // for(size_t i = 20000; i < 20010; ++i){
    //     std::cout << CoM_x_dataset[i] << std::endl;
    //     std::cout << CoM_y_dataset[i] << std::endl;
    //     std::cout << CoM_z_dataset[i] << std::endl;
    //     std::cout << FR_x_dataset[i] << std::endl;
    //     std::cout << FR_y_dataset[i] << std::endl;
    //     std::cout << FR_z_dataset[i] << std::endl;
    ...
    if(con_count % (int) (PongBot.tasktime * onesecScale) == 0){
        ...
        switch (PongBot.CommandFlag){
            ...
            case TROT:
                ...
                //** traj test
                PongBot.walk_para_array.gait_on_off_target = true;
                
                PongBot.walk_para_array.v_CoM_joystick(0) = PongBot.walking.InputData[AXIS_X] * 0.5;
                // PongBot.walk_para_array.v_CoM_joystick(1) = PongBot.walking.InputData[AXIS_Y] * 0.05;
                PongBot.walk_para_array.v_CoM_joystick(1) = PongBot.walking.InputData[AXIS_Y] * 0.1;
                PongBot.walk_para_array.v_CoM_joystick(2) = PongBot.walking.InputData[AXIS_YAW] * 0.2;
                
                PongBot.LocomotionPatternGenerator(PongBot.walk_para_array); // 실험적으로 만드는 새 궤적, 궤적 정보는 walk_para_array 구조체에 포함
                
#if 1
                PongBot.com.G_RefPos        = PongBot.walk_para_array.p_CoM_traj;
                PongBot.FL_Foot.G_RefPos    = PongBot.walk_para_array.p_FL_traj;
                PongBot.FR_Foot.G_RefPos    = PongBot.walk_para_array.p_FR_traj;
                PongBot.RL_Foot.G_RefPos    = PongBot.walk_para_array.p_RL_traj;
                PongBot.RR_Foot.G_RefPos    = PongBot.walk_para_array.p_RR_traj;

                PongBot.base.G_RefOri = PongBot.walk_para_array.ori_CoM_traj;
                
                PongBot.contact.Foot << static_cast<float>(PongBot.walk_para_array.FL_contact),
                                        static_cast<float>(PongBot.walk_para_array.FR_contact),
                                        static_cast<float>(PongBot.walk_para_array.RL_contact),
                                        static_cast<float>(PongBot.walk_para_array.RR_contact);

                // static size_t count_i = 0;
                
                // PongBot.base.G_RefOri(AXIS_PITCH) = static_cast<float>( -0.00005*count_i );
...
// //                    PongBot.gain.JointKp(i*3+0) = 200. * (-1.*(PongBot.contact.Foot(i) - 1.));
// //                    PongBot.gain.JointKp(i*3+1) = 300. * (-1.*(PongBot.contact.Foot(i) - 1.));
// //                    PongBot.gain.JointKp(i*3+2) = 300. * (-1.*(PongBot.contact.Foot(i) - 1.));
// //                
// //                    PongBot.gain.JointKd(i*3+0) = 5. * (-1.*(PongBot.contact.Foot(i) - 1.));
// //                    PongBot.gain.JointKd(i*3+1) = 8. * (-1.*(PongBot.contact.Foot(i) - 1.));
// //                    PongBot.gain.JointKd(i*3+2) = 8. * (-1.*(PongBot.contact.Foot(i) - 1.));
                    
//                     PongBot.gain.JointKp(i*3+0) = 200.;
//                     PongBot.gain.JointKp(i*3+1) = 300.;
//                     PongBot.gain.JointKp(i*3+2) = 300.;
                
//                     PongBot.gain.JointKd(i*3+0) = 8.;
//                     PongBot.gain.JointKd(i*3+1) = 8.;
//                     PongBot.gain.JointKd(i*3+2) = 8.;
//                 }
#endif
        ...
        if(PongBot.osqp.Flag == true){
            ...
            #if 0
            // LocomotionPatternGenerator
            static unsigned int inner_count = 0;
            ++inner_count;
            PongBot.walk_para_array[0].dt_gait = 0.02;

            if(time > 5.){
                if(inner_count % 20 == 1){
                    PongBot.walk_para_array[0].gait_on_off_target = true;
                    // PongBot.walk_para_array[0].v_CoM_joystick(0) = 0.3;
                    PongBot.walk_para_array[0].v_CoM_joystick(0) = 0.;
                    // PongBot.walk_para_array[0].v_CoM_joystick(2) = 0.3;
                    
                    // PongBot.walk_para_array[0].gait_type_target = MOVE_WALK;
                    PongBot.walk_para_array[0].gait_type_target = MOVE_TROT;


                    /* 궤적 생성 */
                    PongBot.LocomotionPatternGenerator(PongBot.walk_para_array[0]); // 이제 이 알고리즘을 여기가 아니라 Trot 플래그 실행 시 구동되도록 옮겨야겠다
                    
                    for(size_t i = 1; i < horizontal_N; ++i){
                        PongBot.walk_para_array[i] = PongBot.walk_para_array[i-1];
                        // float future_time = 0.02;   // 20ms
                        PongBot.walk_para_array[i].time_gait += (mpc_sampletime - PongBot.walk_para_array[0].dt_gait);
                        PongBot.LocomotionPatternGenerator(PongBot.walk_para_array[i]);
                    }

                    /* 내가 사용중인 MPC 제어기에 들어가는 변수들에, 위에서 생성한 값들을 대입 */
                    for(size_t i = 0; i < horizontal_N; ++i){
                        //static int discrete_length = static_cast<int>(round(mpc_sampletime / PongBot.tasktime));       // tasktime = 2ms
                        PongBot.temp_com2FL_ref.block<3, 1>(3 * i, 0) = rot_z(PongBot.walk_para_array[i].ori_CoM_traj[2]).inverse()
                                                                 * (PongBot.walk_para_array[i].p_FL_traj - PongBot.walk_para_array[i].p_CoM_traj);
                        PongBot.temp_com2FR_ref.block<3, 1>(3 * i, 0) = rot_z(PongBot.walk_para_array[i].ori_CoM_traj[2]).inverse()
                                                                 * (PongBot.walk_para_array[i].p_FR_traj - PongBot.walk_para_array[i].p_CoM_traj);

                        PongBot.temp_com2RL_ref.block<3, 1>(3 * i, 0) = rot_z(PongBot.walk_para_array[i].ori_CoM_traj[2]).inverse()
                                                                 * (PongBot.walk_para_array[i].p_RL_traj - PongBot.walk_para_array[i].p_CoM_traj);
                        PongBot.temp_com2RR_ref.block<3, 1>(3 * i, 0) = rot_z(PongBot.walk_para_array[i].ori_CoM_traj[2]).inverse()
                                                                 * (PongBot.walk_para_array[i].p_RR_traj - PongBot.walk_para_array[i].p_CoM_traj);

                        PongBot.temp_contact_foots(0, i) = PongBot.walk_para_array[i].FL_contact;
                        PongBot.temp_contact_foots(1, i) = PongBot.walk_para_array[i].FR_contact;
                        PongBot.temp_contact_foots(2, i) = PongBot.walk_para_array[i].RL_contact;
                        PongBot.temp_contact_foots(3, i) = PongBot.walk_para_array[i].RR_contact;

                        PongBot.temp_com_ref.block<3, 1>(3*i, 0) = PongBot.walk_para_array[i].p_CoM_traj;

                        // 미래 다리 좌표계
                        // PongBot.temp_Offset_Global_between_FootCoord = com.G_RefPos - com.G_Foot_to_CoM_RefPos
                        // PongBot.temp_Foot_to_CoM_ref.block<3, 1>(i*3, 0) = 
                        //
                        //temp_Foot_to_CoM_ref
                        //temp_Foot_to_CoM_vel_ref
                    ...
                    // PongBot.walk_para_array[1] = PongBot.walk_para_array[0];
                    // PongBot.walk_para_array[1].time_gait += 0.02;
                    // PongBot.LocomotionPatternGenerator(PongBot.walk_para_array[1]);

                // PongBot.walk_para_array[0].p_FL_traj;
                // PongBot.walk_para_array[1].p_FL_traj;
                // PongBot.walk_para_array[2].p_FL_traj;
                // ...
                }

                // 새로 생성해준 궤적을 기존 퐁봇 변수에 덮어씌워야 함
                PongBot.FL_Foot.G_RefPos = PongBot.walk_para_array[0].p_FL_traj;
                PongBot.FR_Foot.G_RefPos = PongBot.walk_para_array[0].p_FR_traj;
                PongBot.RL_Foot.G_RefPos = PongBot.walk_para_array[0].p_RL_traj;
                PongBot.RR_Foot.G_RefPos = PongBot.walk_para_array[0].p_RR_traj;

                PongBot.com.G_RefPos = PongBot.walk_para_array[0].p_CoM_traj;
                ...
                // std::cout << "walk_paray_array[0] gait time: " << PongBot.walk_para_array[0].time_gait << std::endl;
                // std::cout << "walk_paray_array[1] gait time: " << PongBot.walk_para_array[1].time_gait << std::endl;
                // std::cout << "walk_para_array[0] desired yaw: " << PongBot.walk_para_array[0].ori_CoM_desired.transpose() << std::endl;
                // std::cout << "walk_para_array[0] traj yaw: " << PongBot.walk_para_array[0].ori_CoM_traj.transpose() << std::endl;
                //ori_CoM_traj[2]
                ...
                RefPos_B << rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_FL_traj - PongBot.walk_para_array[0].p_CoM_traj),
                            rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_FR_traj - PongBot.walk_para_array[0].p_CoM_traj),
                            rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_RL_traj - PongBot.walk_para_array[0].p_CoM_traj),
                            rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_RR_traj - PongBot.walk_para_array[0].p_CoM_traj);

                // PongBot.FL_Foot.B_RefPos = rot_z(0.).inverse() * (PongBot.walk_para_array[0].p_FL_traj - PongBot.walk_para_array[0].p_CoM_traj);
                // PongBot.FR_Foot.B_RefPos = rot_z(0.).inverse() * (PongBot.walk_para_array[0].p_FR_traj - PongBot.walk_para_array[0].p_CoM_traj);
                // PongBot.RL_Foot.B_RefPos = rot_z(0.).inverse() * (PongBot.walk_para_array[0].p_RL_traj - PongBot.walk_para_array[0].p_CoM_traj);
                // PongBot.RR_Foot.B_RefPos = rot_z(0.).inverse() * (PongBot.walk_para_array[0].p_RR_traj - PongBot.walk_para_array[0].p_CoM_traj);

                PongBot.FL_Foot.B_RefPos = rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_FL_traj - PongBot.walk_para_array[0].p_CoM_traj);
                PongBot.FR_Foot.B_RefPos = rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_FR_traj - PongBot.walk_para_array[0].p_CoM_traj);
                PongBot.RL_Foot.B_RefPos = rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_RL_traj - PongBot.walk_para_array[0].p_CoM_traj);
                PongBot.RR_Foot.B_RefPos = rot_z(PongBot.walk_para_array[0].ori_CoM_traj[2]).inverse() * (PongBot.walk_para_array[0].p_RR_traj - PongBot.walk_para_array[0].p_CoM_traj);
                

                // std::cout << "PongBot.walk_para_array[0].p_FL_traj: " << PongBot.walk_para_array[0].p_FL_traj.transpose() << std::endl;
                // std::cout << "PongBot.walk_para_array[0].p_CoM_traj: " << PongBot.walk_para_array[0].p_CoM_traj.transpose() << std::endl;
                // std::cout << "FL_Foot.B_RefPos: " << PongBot.FL_Foot.B_RefPos.transpose() << std::endl;

                // RefPos_B << rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_FL_traj - PongBot.walk_para_new.p_CoM_traj),
                //             rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_FR_traj - PongBot.walk_para_new.p_CoM_traj),
                //             rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_RL_traj - PongBot.walk_para_new.p_CoM_traj),
                //             rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_RR_traj - PongBot.walk_para_new.p_CoM_traj);

                // PongBot.FL_Foot.B_RefPos = rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_FL_traj - PongBot.walk_para_new.p_CoM_traj);
                // PongBot.FR_Foot.B_RefPos = rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_FR_traj - PongBot.walk_para_new.p_CoM_traj);
                // PongBot.RL_Foot.B_RefPos = rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_RL_traj - PongBot.walk_para_new.p_CoM_traj);
                // PongBot.RR_Foot.B_RefPos = rot_z(yaw_traj).inverse() * (PongBot.walk_para_new.p_RR_traj - PongBot.walk_para_new.p_CoM_traj);
                ...
                // std::cout << "PongBot.joint[2].RefPos: " << PongBot.joint[2].RefPos << std::endl;
            ...
            if(inner_count % 20 == 1){
                if(time <= 5.){
                    for(size_t i = 0; i < horizontal_N; ++i){
                        PongBot.temp_contact_foots(0, i) = true;
                        PongBot.temp_contact_foots(1, i) = true;
                        PongBot.temp_contact_foots(2, i) = true;
                        PongBot.temp_contact_foots(3, i) = true;
                        PongBot.temp_com2FL_ref.block<3, 1>(3 * i, 0) = PongBot.walk_para_array[0].p_com2FL_offset;
                        PongBot.temp_com2FR_ref.block<3, 1>(3 * i, 0) = PongBot.walk_para_array[0].p_com2FR_offset;
                        PongBot.temp_com2RL_ref.block<3, 1>(3 * i, 0) = PongBot.walk_para_array[0].p_com2RL_offset;
                        PongBot.temp_com2RR_ref.block<3, 1>(3 * i, 0) = PongBot.walk_para_array[0].p_com2RR_offset;
                        PongBot.temp_com_ref.block<3, 1>(3* i, 0) = PongBot.walk_para_array[0].p_CoM_desired;
                    }
                }
                PongBot.ProcessOSQP();
            }

            // std::cout << "Stance_On_flag: " << PongBot.walk_para_array[0].Stance_On_flag << std::endl;
            // std::cout << "Swing_On_flag: " << PongBot.walk_para_array[0].Swing_On_flag << std::endl;
            // std::cout << "swingLeg: " << PongBot.walk_para_array[0].swingLeg << std::endl;
            // std::cout << "v_CoM_desired: " << PongBot.walk_para_array[0].v_CoM_desired.transpose() << std::endl;
            // std::cout << "p_CoM_desired: " << PongBot.walk_para_array[0].p_CoM_desired.transpose() << std::endl;
            // std::cout << "p_FL_desired: " << PongBot.walk_para_array[0].p_FL_desired.transpose() << std::endl;
    ...
    */
    ...
    if(JoyMode == SERIAL)
    {
        if (msg->buttons[3] == true) {
            ...
            if(PongBot.walk_para_array.gait_type == MOVE_WALK){
                PongBot.walk_para_array.gait_type_target = MOVE_TROT;
                std::cout << "WALK MODE -> TROT MODE" << std::endl;
                
            }
            else if(PongBot.walk_para_array.gait_type == MOVE_TROT){
                PongBot.walk_para_array.gait_type_target = MOVE_WALK;
                std::cout << "TROT MODE -> WALK MODE" << std::endl;
            }

            //** 이 버튼의 기능을 보행천이 대신 단차보행을 위한 엔드포인트 높이 조절용으로 썼음
            // static size_t static_count = 0;
            // if(PongBot.walk_para_array.gait_type == MOVE_WALK){
            //     if(static_count == 0){
            //         PongBot.walk_para_array.FL_footstep_pos_flag = true;
            //         PongBot.walk_para_array.FL_footstep_pos(2) = 0.3;
            //         PongBot.walk_para_array.FR_footstep_pos_flag = true;
            //         PongBot.walk_para_array.FR_footstep_pos(2) = 0.3;
            //         ++static_count;
            //     }
            //     else{
            //         PongBot.walk_para_array.RL_footstep_pos_flag = true;
            //         PongBot.walk_para_array.RL_footstep_pos(2) = 0.3;
            //         PongBot.walk_para_array.RR_footstep_pos_flag = true;
            //         PongBot.walk_para_array.RR_footstep_pos(2) = 0.3;
            //     }
            // }
            
//             if((PongBot.walking.Ready == true)&&(PongBot.walking.MoveDone == true)){
// //                PongBot.ControlMode             = CTRLMODE_DATAMOTION;
// //                PongBot.CommandFlag             = DATA_MOTION;
