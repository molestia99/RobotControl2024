...
void gazebo::PongBot_plugin::Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/) {    
    ...
    //** 프로그램 처음 로딩 시 contact 정보를 전부 true 로 저장
    for (size_t i = 0; i < horizontal_N; ++i){
        ...
        PongBot.temp_com2FL_ref.block<3, 1>(3*i, 0) << 0.340, 0.212, -0.4;
        PongBot.temp_com2FR_ref.block<3, 1>(3*i, 0) << 0.340, -0.212, -0.4;
        PongBot.temp_com2RL_ref.block<3, 1>(3*i, 0) << -0.340, 0.212, -0.4;
        PongBot.temp_com2RR_ref.block<3, 1>(3*i, 0) << -0.340, -0.212, -0.4;
...
//                 }
                ...
                PongBot.walk_para_array[0].v_CoM_joystick(0) = PongBot.walking.InputData[AXIS_X] * 0.5;                
                PongBot.walk_para_array[0].v_CoM_joystick(1) = PongBot.walking.InputData[AXIS_Y] * 0.1;
                PongBot.walk_para_array[0].v_CoM_joystick(2) = PongBot.walking.InputData[AXIS_YAW] * 0.2;
                
                PongBot.LocomotionPatternGenerator(PongBot.walk_para_array[0]); // 실험적으로 만드는 새 궤적, 궤적 정보는 walk_para_array 구조체에 포함

                for(size_t i = 1; i < horizontal_length; ++i){
                    PongBot.walk_para_array[i] = PongBot.walk_para_array[i-1];
                    
                    //** 미래시간 더하기
                    PongBot.walk_para_array[i].time_gait += mpc_sampletime - dt;
                    PongBot.LocomotionPatternGenerator(PongBot.walk_para_array[i]);
                }

                PongBot.contact.Foot(0) = PongBot.walk_para_array[0].FL_contact;
                PongBot.contact.Foot(1) = PongBot.walk_para_array[0].FR_contact;
                PongBot.contact.Foot(2) = PongBot.walk_para_array[0].RL_contact;
                PongBot.contact.Foot(3) = PongBot.walk_para_array[0].RR_contact;
                ...
                PongBot.base.G_RefOri = PongBot.walk_para_array[0].ori_CoM_traj;
                
                PongBot.contact.Foot << static_cast<float>(PongBot.walk_para_array[0].FL_contact),
                                        static_cast<float>(PongBot.walk_para_array[0].FR_contact),
                                        static_cast<float>(PongBot.walk_para_array[0].RL_contact),
                                        static_cast<float>(PongBot.walk_para_array[0].RR_contact);
                
                // future
                for(size_t i = 0; i < horizontal_length; ++i){

                    //PongBot.EulerZYX2RotMat(PongBot.walk_para_array[i].ori_CoM_traj)

                    PongBot.temp_base_ori_ref.block<3, 1>(i*3, 0) = PongBot.walk_para_array[i].ori_CoM_traj;

                    PongBot.temp_com_ref.block<3, 1>(i * 3, 0) = PongBot.walk_para_array[i].p_CoM_traj;
                    PongBot.temp_legs_ref.block<3, 1>(i * 3, 0) = PongBot.walk_para_array[i].p_FL_traj;
                    PongBot.temp_legs_ref.block<3, 1>(i * 3, 1) = PongBot.walk_para_array[i].p_FR_traj;
                    PongBot.temp_legs_ref.block<3, 1>(i * 3, 2) = PongBot.walk_para_array[i].p_RL_traj;
                    PongBot.temp_legs_ref.block<3, 1>(i * 3, 3) = PongBot.walk_para_array[i].p_RR_traj;

                    // 몸체의 방위자세에 대한 고려가 없음
                    PongBot.temp_com2FL_ref.block<3, 1>(i * 3, 0) = PongBot.temp_legs_ref.block<3, 1>(i * 3, 0) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0);
                    PongBot.temp_com2FR_ref.block<3, 1>(i * 3, 0) = PongBot.temp_legs_ref.block<3, 1>(i * 3, 1) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0);
                    PongBot.temp_com2RL_ref.block<3, 1>(i * 3, 0) = PongBot.temp_legs_ref.block<3, 1>(i * 3, 2) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0);
                    PongBot.temp_com2RR_ref.block<3, 1>(i * 3, 0) = PongBot.temp_legs_ref.block<3, 1>(i * 3, 3) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0);
                    
                    // 몸체의 방위자세에 대해 고려한다면
                    // PongBot.temp_com2FL_ref.block<3, 1>(i * 3, 0) = PongBot.EulerZYX2RotMat(PongBot.walk_para_array[i].ori_CoM_traj)*(PongBot.temp_legs_ref.block<3, 1>(i * 3, 0) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0));
                    // PongBot.temp_com2FR_ref.block<3, 1>(i * 3, 0) = PongBot.EulerZYX2RotMat(PongBot.walk_para_array[i].ori_CoM_traj)*(PongBot.temp_legs_ref.block<3, 1>(i * 3, 1) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0));
                    // PongBot.temp_com2RL_ref.block<3, 1>(i * 3, 0) = PongBot.EulerZYX2RotMat(PongBot.walk_para_array[i].ori_CoM_traj)*(PongBot.temp_legs_ref.block<3, 1>(i * 3, 2) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0));
                    // PongBot.temp_com2RR_ref.block<3, 1>(i * 3, 0) = PongBot.EulerZYX2RotMat(PongBot.walk_para_array[i].ori_CoM_traj)*(PongBot.temp_legs_ref.block<3, 1>(i * 3, 3) - PongBot.temp_com_ref.block<3, 1>(i * 3, 0));
                    
                    if(i > 0){
                        PongBot.temp_com_vel_ref.block<3, 1>(i * 3, 0) = (PongBot.temp_com_ref.block<3, 1>(i * 3, 0) - PongBot.temp_com_ref.block<3, 1>((i - 1) * 3, 0)) / (float)(2.0 / onesecScale);
                    }
                    ...
                    if(i == 0){
                        PongBot.temp_com2FL_ref.block<3, 1>(i * 3, 0) = PongBot.FL_Foot.G_CoM_to_Foot_CurrentPos;
                        PongBot.temp_com2FR_ref.block<3, 1>(i * 3, 0) = PongBot.FR_Foot.G_CoM_to_Foot_CurrentPos;
                        PongBot.temp_com2RL_ref.block<3, 1>(i * 3, 0) = PongBot.RL_Foot.G_CoM_to_Foot_CurrentPos;
                        PongBot.temp_com2RR_ref.block<3, 1>(i * 3, 0) = PongBot.RR_Foot.G_CoM_to_Foot_CurrentPos;
                ...
                // std::cout << "PongBot.temp_com2FL_ref: " << PongBot.temp_com2FL_ref.transpose() << std::endl;
                // std::cout << "real p_FL:         " << PongBot.FL_Foot.G_CoM_to_Foot_CurrentPos.transpose() << std::endl;
        ...
        if(PongBot.osqp.Flag == true){
            ...
            static unsigned int count_temp = 0;
    ...
    */
    ...
    if(JoyMode == SERIAL)
    {
        if (msg->buttons[3] == true) {
            ...
            if(PongBot.walk_para_array[0].gait_type == MOVE_WALK){
                PongBot.walk_para_array[0].gait_type_target = MOVE_TROT;
                std::cout << "WALK MODE -> TROT MODE" << std::endl;
                
            }
            else if(PongBot.walk_para_array[0].gait_type == MOVE_TROT){
