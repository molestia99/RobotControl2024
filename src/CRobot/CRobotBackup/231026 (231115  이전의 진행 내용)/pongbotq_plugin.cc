        ...
        if(PongBot.osqp.Flag == true){
            ...
            if(time > 5.){
                ...
                if(time <= 600.){
                    ...
                    PongBot.walk_para_new.p_CoM_traj << 0., 0., 0.4;
                    // size_t index = static_cast<size_t>(i /3);
                    PongBot.walk_para_new.p_CoM_traj(AXIS_X) = CoM_x_dataset[i] * 1.;
                    PongBot.walk_para_new.p_CoM_traj(AXIS_Y) = CoM_y_dataset[i] * 1.;

                    PongBot.walk_para_new.p_FL_traj << FL_x_dataset[i], FL_y_dataset[i], FL_z_dataset[i];
                    PongBot.walk_para_new.p_FR_traj << FR_x_dataset[i], FR_y_dataset[i], FR_z_dataset[i];
                    PongBot.walk_para_new.p_RL_traj << RL_x_dataset[i], RL_y_dataset[i], RL_z_dataset[i];
                    PongBot.walk_para_new.p_RR_traj << RR_x_dataset[i], RR_y_dataset[i], RR_z_dataset[i];

                    yaw_traj = yaw_dataset[i]; // rad

                    ++i;
                    // if(i > 58429) i = 58429;                   // 500 * 3

                    if(i > 57999) i = 57999;                   // 500 * 3


                    // PongBot.walk_para_new.p_FL_traj = PongBot.walk_para_new.p_com2FL_offset;
                    // PongBot.walk_para_new.p_FR_traj = PongBot.walk_para_new.p_com2FR_offset;
                    // PongBot.walk_para_new.p_RL_traj = PongBot.walk_para_new.p_com2RL_offset;
                    // PongBot.walk_para_new.p_RR_traj = PongBot.walk_para_new.p_com2RR_offset;

                    // PongBot.walk_para_new.p_FL_traj(AXIS_Z) = 0.;
                    // PongBot.walk_para_new.p_FR_traj(AXIS_Z) = 0.;
                    // PongBot.walk_para_new.p_RL_traj(AXIS_Z) = 0.;
                    // PongBot.walk_para_new.p_RR_traj(AXIS_Z) = 0.;
                }                
